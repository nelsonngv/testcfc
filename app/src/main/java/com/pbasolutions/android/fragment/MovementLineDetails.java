...
public class MovementLineDetails extends AbstractMovementLineFragment {
    ...
    protected MMovementLine getMovementLine() {
        ...
        input.putInt(PBSAssetController.ARG_M_MOVEMENTLINE_ID, get_ID());
        input.putSerializable(PBSAssetController.ARG_MOVEMENTLINES, getModelList());
        Bundle result = assetCont.triggerEvent(PBSAssetController.GET_MOVEMENTLINE_EVENT, input,
                new Bundle(), null);
        return (MMovementLine)result.getSerializable(PBSAssetController.ARG_MOVEMENTLINE);
    }
    ...
    protected List<SpinnerPair> getAsi() {
        Bundle input = new Bundle();
        input.putString(PBSAssetController.ARG_M_PRODUCT_UUID, ((line.getM_Product_UUID())));
    ...
    public void setValues(){
        ...
        if (prodNameAdapter.getCount()>0){
            ...
            PandoraHelper.showErrorMessage(getActivity(), "You have not full sync Asset" +
    ...
    public void save() {
        ...
        Bundle output = assetCont.triggerEvent(PBSAssetController.UPDATE_MOVEMENTLINE_EVENT, input, new Bundle(), null);
