...
public class CheckPointFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (globalVar.getProjLocJSON() == null) {
            //find in database.
            Bundle result = checkpointController.triggerEvent(checkpointController.GET_PROJECT_LOCATION_DATA, new Bundle(), new Bundle(), null);
            ...
            PandoraHelper.showAlertMessage((PandoraMain)getActivity(), result.getString(result.getString(PandoraConstant.TITLE)), result.getString(PandoraConstant.TITLE), "Ok", null);

            recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));
            CheckPointRVA viewAdapter = new CheckPointRVA(getActivity(),null, inflater);
            recyclerView.setAdapter(viewAdapter);
        } else {
            recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));
            CheckPointRVA viewAdapter = new CheckPointRVA(getActivity(),getCheckPointSeqList(), inflater);
            recyclerView.setAdapter(viewAdapter);
        }
        return rootView;
    }
